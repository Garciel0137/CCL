...
class sheet {
  display() {
    img4.loadPixels(); // ensure img4 is loaded in preload()
...
It was the least we could do. And it was what the ocean deserved.`,
        ...
        text(
`Before departure, people all 
around the world took part in a 
final volunteer cleanup:
...
On the final day, all the cleanup ships sounded their horns at dusk. Our work wasn’t finished, but time had run out. We had to leave.`,
        ...
        text(
`The tips of the waves held a rust-colored silence.
...
If I claimed I wasn't glad nature was finally rid of us, I’d be lying.`,
          120,200,250
        );
      }
      else if (bottlenum <= 9) {
        text(
`The horizon was littered with rib-like shadows—bloated tires pressing against one another. With every tide, a few water bottles and plastic sandals would be squeezed free, spreading across the rocks like a festering imprint.`,120,200,180)

text(`It was impossible to tell whether the floating shapes were trash or living things. They were massive and disturbing, and as the wind moved them, they seemed almost sentient.

The silence after low tide was the loudest of all. Swarms of plastic bottles shimmered silver beneath the sea surface, replacing the sardines that once filled these waters.

This is what the 21st century left behind: a postcard we can never bear to read again.`,
          120,355,250
        );
      }
      else {  // bottlenum > 9
...
class fish {
  ...
  update() {
    this.x += cos(this.angle) * 1.2;
    this.y += sin(this.angle) * 1.2;
...
class bottle {
  ...
  update() {
    this.time += 0.05;
    this.x += sin(this.time*0.3)*1.2
    this.y += sin(this.time)*0.2
  }

  display() {
    push();
    translate(this.x, this.y);
    let sway = sin(this.time) * 5;
    rotate(radians(sway));
...
function keyPressed() {
  if (key === "F" || key === "f") {
    ...
    if (key === "B" || key === "b") {
    bottlenum++;
    BottleList.push(new bottle(random(0,500),random(160,500)));
  } 
  else if ((key === "X" || key === "x") && bottlenum > 0) {
    bottlenum--;
    let indexToRemove = floor(random(BottleList.length));
    BottleList.splice(indexToRemove, 1);
  }
}
